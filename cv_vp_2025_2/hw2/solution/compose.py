...
def compose_test(notebook_path, output_path):
    ...
    for name in _expected_graded:
        if name not in graded_source:
            raise RuntimeError(f"Not enough graded functions, {name} is missing (or {name} may contain an IndentationError\n {errs})")
...
def main():
    ...
    parser.add_argument('--notebook', type=str, default="super_resolution.ipynb")
    parser.add_argument('--output', type=str, default="sr.py")
