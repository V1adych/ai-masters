...
class DecisionTree:
    ...
    def _build_nodes(
        ...
        if (
            self.min_improvement is not None
            and self.min_improvement >= node.q_value_max
        ):
            node.set_predictions(criterion.get_predict_val(y[indices]))
            return
...
class ClassificationDecisionTree(DecisionTree):
    ...
    def feature_importance_(self) -> np.ndarray:
        ...
        feature_q_values = np.zeros(self.n_features)

        def get_feature_importance(node: TreeNode):
            if node.is_terminal():
                return
            feature_q_values[node.feature_id] += node.q_value_max
            get_feature_importance(node.left_child)
            get_feature_importance(node.right_child)

        get_feature_importance(self.root)

        return feature_q_values / feature_q_values.sum()
